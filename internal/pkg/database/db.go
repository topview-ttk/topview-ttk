...
func SaveSqlJoins[T any](data *T, table string) string {
	...
	for i := 0; i < fieldNum; i++ {
		...
		if colType == "int64" {
			...
			if !value.IsZero() {
				names += fmt.Sprintf("`%s`,", typ.Field(i).Tag.Get("db"))
				values += fmt.Sprintf("'%s',", value.Format(date.DateTimeFormat))
			}
		}
	}
...
func EditSqlJoins[T any](data *T, table string, Id int64) string {
	...
	for i := 1; i < fieldNum; i++ {
		...
		if colType == "int64" {
			...
			value := Field.Interface().(time.Time)
			if !value.IsZero() {
				names += fmt.Sprintf("`%s`='%s',", typ.Field(i).Tag.Get("db"), value.Format(date.DateTimeFormat))
			}
		}
	}
	names = strings.TrimRight(names, ",")
...
func TransCtx(ctx context.Context, conn sqlx.SqlConn, fn func(ctx context.Context, session sqlx.Session) error) error {
