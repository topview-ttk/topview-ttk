...
import (
	"fmt"
	"time"
	stringttk "topview-ttk/internal/pkg/string"
)
...
func TimeList(startDate string, endDate string, dateType string, interval int) []string {
	...
	if dateType == "second" {
		...
		for endTime.After(startTime) {
			date := stringttk.Substring(startTime.Format(layout), 10)
			array = append(array, date)
			startTime = startTime.AddDate(0, 0, interval)
		}
		end := stringttk.Substring(endTime.Format(layout), 10)
		if array[len(array)-1] != end {
			array = append(array, end)
		}
	} else if dateType == "month" {
		for endTime.After(startTime) {
			date := stringttk.Substring(startTime.Format(layout), 7)
			array = append(array, date)
			startTime = startTime.AddDate(0, 1, 0)
		}
		end := stringttk.Substring(endTime.Format(layout), 7)
		if array[len(array)-1] != end {
			array = append(array, end)
		}
	} else if dateType == "year" {
		for endTime.After(startTime) {
			date := stringttk.Substring(startTime.Format(layout), 4)
			array = append(array, date)
			startTime = startTime.AddDate(interval, 0, 0)
		}
		end := stringttk.Substring(endTime.Format(layout), 4)
		if array[len(array)-1] != end {
			array = append(array, end)
		}
	}
